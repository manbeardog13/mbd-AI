#!/usr/bin/env python3
"""Offline verification for Nero Core and Mission Control Milestone 1."""
...
def static_contract() -> None:
    ...
    check(
        "Codex startup hooks remain empty",
        hooks == {} or hooks == {"hooks": {}},
    )
...
def run_suite() -> None:
    ...
    check("Core, Git, orchestration, API, and interface suites", result.returncode == 0)


def main() -> int:
    print("Nero Mission Control M1 — offline verification")
    static_contract()
    run_suite()
    print()
    if FAILURES:
        ...
        return 1
    print("  Mission Control M1 verified. No hosted worker, local model, or remote write was invoked.")
    return 0
