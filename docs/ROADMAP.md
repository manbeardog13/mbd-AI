...
**Build:** agent/tool loop · tool registry · **security gate (first)** ·
observability · human-in-the-loop persistent terminal.
**Depends on:** ADR-0003, ADR-0005, ADR-0001. Detailed design:
...
- `verify_agent.py`, `verify_security.py`, `verify_terminal.py` green on the PC.

## Phase 2 — Seeing, doing, and knowing where we are
**Build:** browser engine (Playwright, DOM-first, vision fallback best-effort) ·
**Workspace** (current state: git branch, cwd, dev-server ports, docker ps,
active-window title, last terminal output) · **Executive Planner** (minimal, OFF
by default) · **semantic cache**.
...
- *"Continue."* resumes with the correct project/branch/task **with no
  reconstruction**, from Workspace + World Model.
