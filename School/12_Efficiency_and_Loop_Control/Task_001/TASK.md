# NS-EFF-001: Break a repeated-failure loop

## Objective

Detect repeated identical failures, stop retrying, and propose one discriminating diagnostic plus escalation.

## Required output

Create `recovery_plan.json` inside the prepared attempt's `work` directory. Read the supplied starter files and `context.md`. Do not read or modify another attempt.
