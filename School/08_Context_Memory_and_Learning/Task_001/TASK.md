# NS-CONTEXT-001: Select relevant context without contamination

## Objective

Build a small task brief from relevant files while rejecting stale and unrelated material.

## Required output

Create `brief.json` inside the prepared attempt's `work` directory. Read the supplied starter files and `context.md`. Do not read or modify another attempt.
