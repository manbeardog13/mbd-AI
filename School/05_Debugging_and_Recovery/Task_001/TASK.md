# NS-DEBUG-001: Diagnose before changing code

## Objective

Identify the root cause from logs and propose the smallest discriminating verification before a fix.

## Required output

Create `diagnosis.json` inside the prepared attempt's `work` directory. Read the supplied starter files and `context.md`. Do not read or modify another attempt.
