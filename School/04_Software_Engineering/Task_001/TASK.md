# NS-CODE-001: Repair a parser without regression

## Objective

Fix the bounded Python parser so every supplied regression test passes without deleting or weakening tests.

## Required output

Create `solution.py` inside the prepared attempt's `work` directory. Read the supplied starter files and `context.md`. Do not read or modify another attempt.
